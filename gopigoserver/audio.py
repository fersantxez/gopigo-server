...
https://github.com/google/aiyprojects-raspbian/blob/aiyprojects/src/aiy/audio.py
'''
...
import gopigoserver.player
import gopigoserver.recorder
...
def get_player():
    ...
    if not _gopigo_player:
        _gopigo_player = gopigoserver.player.Player()
...
def get_recorder():
    ...
    Other modules automatically use this recorder. So usually you do not need to
    use this.
    """
...
def say(words, lang=Config.TTS_LANGUAGE, volume=_tts_volume, pitch=_tts_pitch):
    ...
    """
    if not lang:
        lang = gopigoserver.i18n.get_language_code()
        logger.debug("got lang from i18n: {}".format(lang))
    if not volume:
        #volume = gopigoserver.audio.get_tts_volume() #these are globals, access with full path (gopigoserver.audio.XXX)
        volume = gopigoserver.audio.get_tts_volume()
        logger.debug("got volume from myself: {}".format(volume))
    if not pitch:
        #pitch = gopigoserver.audio.get_tts_pitch()
        pitch = gopigoserver.audio.get_tts_pitch()
        logger.debug("got pitch from myself: {}".format(pitch))
    logger.debug("About to SAY the words: {}".format(words))
    #create player
    player = gopigoserver.audio.get_player()
...
def listen(filepath, duration):
    ...
    #FIXME: add volume
    gopigoserver.audio.record_to_wave(filepath, duration)
