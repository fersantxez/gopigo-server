...
right_rot_cmd = [110
                 ]  #Rotate Right by running both motors is opposite direction
...
def enc_tgt(m1, m2, target):
    if m1 > 1 or m1 < 0 or m2 > 1 or m2 < 0:
        return -1
    m_sel = m1 * 2 + m2
    write_i2c_block(address,
                    enc_tgt_cmd + [m_sel, target // 256, target % 256])
    return 1
...
def enable_com_timeout(timeout):
    return write_i2c_block(
        address, en_com_timeout_cmd + [timeout // 256, timeout % 256, 0])
...
def dht(sensor_type=0):
    try:
        ...
        humidity, temperature = Adafruit_DHT.read_retry(
            sensor, pin, retries=3, delay_seconds=.1)
