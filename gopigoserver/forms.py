...
class FormRegistration(FlaskForm):
    ...
    password2 = PasswordField(
        'Repeat Password (will be ignored for OAuth users)',
...
class FormSettings(FlaskForm):
    camera_res = SelectField(
        'Camera Resolution (horizontal)', choices=Config.CAMERA_RES_LIST)
...
class FormVision(FlaskForm):
    feature = SelectField(
        'What do you want to detect in the picture?',
        choices=Config.VISION_API_FEATURES_LIST)
    submit = SubmitField('Submit')
