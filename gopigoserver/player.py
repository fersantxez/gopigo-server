...
https://github.com/google/aiyprojects-raspbian/blob/aiyprojects/src/aiy/_drivers/_player.py
'''
...
class Player(object):
    ...
    def play_bytes(self, audio_bytes, sample_rate, sample_width=2):
        ...
        logger.debug('Played bytes, about to CLOSE')
        aplay.stdin.close()
        logger.debug('aplay closed, wait to retcode')
        retcode = aplay.wait()
        logger.debug('DONE. about to exit')
        if retcode:
    ...
    def play_wav(self, wav_path):
        ...
        """
        logger.debug('About to play WAV file: {}'.format(wav_path))
