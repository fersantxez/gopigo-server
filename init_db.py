...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '-u', '--username', help='username to login with', required=False)
    parser.add_argument(
        '-p', '--password', help='password to login with', required=False)
    ...
    username = args.get('username') or input(
        "Please enter username to initialize the App with: ")
    password = args.get('password') or input(
        "Please enter password for user {}: ".format(username))

    logger.debug(
        '**DEBUG: about to create a Superuser named {} with password {}'.
    ...
    if User.query.filter_by(username=admin_user.username).first():
        logger.error('**ERROR: USER {} EXISTS. EXITING'.format(
            admin_user.username))
    ...
    for i in range(1, 4):
        empty_picture_full_path = os.path.join(Config.MEDIA_DIR,
                                               Config.EMPTY_PICTURE)
        document = create_document_from_file(
            empty_picture_full_path, type="picture", user_id=admin_user.id)
        if document:
            logger.debug("**DEBUG: Created document {} with name {}".format(
